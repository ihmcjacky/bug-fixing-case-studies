...
// import P2FwUpActPanel from '../../../components/nodeMaintenances/P2FwUpActPanel';
...
const MeshWideMaintenanceFwUpgrade = (props) => {
    const {
        isInit,
        tableData,
        refreshFunc,
    ...
    useEffect(() => {
        if (seriesList.includes('ax50')) {
            setAX50FileUpload({
                ...ax50FileUpload,
                display: true,
            });
        }
        if (seriesList.includes('x30')) {
            setX30FileUpload({
                ...x30FileUpload,
                display: true,
            });
        }
        if (seriesList.includes('x20')) {
            setX20FileUpload({
                ...x20FileUpload,
                display: true,
            });
        }
        if (seriesList.includes('x10')) {
            setX10FileUpload({
                ...x10FileUpload,
                display: true,
            });
        }
        if (seriesList.includes('z500')) {
            setZ500FileUpload({
                ...z500FileUpload,
                display: true,
            });
    ...
    const onFileSelect = (event, series) => {
        ...
        if (filesize > Constants.fwSizeLimit[series]) {
            setDialog({
                open: true,
    ...
    return (
        <>
            ...
            <P2MultipleFwUpgradeActPanel
                ...
                disabledUpgrade={disableUpgradeBtn || hasNodeUpgrading || hasUnreachable}
                style={{
                    panelDiv: {
                        width: '100%',
                ...
                fwFileName={fileSelect.fileName}
                fwFileSize={fileSelect.fileSize}
                disabledSelectFW={fileSelect.lock ||  hasNodeUpgrading || hasUnreachable}
...
MeshWideMaintenanceFwUpgrade.propTypes = {
...
export default MeshWideMaintenanceFwUpgrade;
