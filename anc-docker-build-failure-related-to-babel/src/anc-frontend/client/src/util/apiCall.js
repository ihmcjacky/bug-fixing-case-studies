...
function handleSessionExpied(
    ...
    console.log('------handleSessionExpied');
...
function generalResponseHandler(res, ctx, notDirect = false, returnKey = false) {
    ...
    console.log('checkpoint 4', ctx, res)
...
export async function getInitData() {
    try {
        // const res = await restfulUiApiCall('/get-init-data');
        // const ctx = await res.json();

        // return generalResponeHandler(res, ctx, false);

        // const env = {
        //     UI_DISPLAY_VER: process.env.UI_DISPLAY_VER,
        //     AOS_SUPPORTED_VER: process.env.AOS_SUPPORTED_VER,
        //     NTP_SERVER_VER: process.env.NTP_SERVER_VER,
        //     CM_VER: process.env.CM_VER,
        //     CA_VER: process.env.CA_VER,
        // };
        return new Promise((resolve, reject) => {
            ...
            request.onload = function () {
                ...
                console.log('emulateDom', emulateDom)
...
export async function linkAlignmentExportXLS(csrf, bodyMsg) {
    try {
        ...
        console.log('kyle_debug: linkAlignmentExportXLS -> res', res)
        const ctx = await res.blob();
        console.log('kyle_debug: linkAlignmentExportXLS -> ctx', ctx)
...
export async function logoutProject(csrf, body, projectId, signal = null) {
    try {
        ...
        console.log('logoutProject api call')
        console.log(res)
        const ctx = await res.json();
        console.log(ctx)
        return generalResponseHandler(res, ctx);
...
export async function getProjectDebugInfo(csrf, projectId, body) {
        ...
        if (res.status === 200) {
            console.log('------kyle--getProjectDebugInfo(start blob)-----');
            const ctx = await res.blob();
            console.log('------kyle--getProjectDebugInfo(finish blob)-----');
            const re = /filename="(\S+)"/;
            console.log(res.headers.get('Content-Disposition').match(re));
...
export async function getDebugInfo(csrf) {
    try {
        ...
        if (res.status === 200) {
            console.log('------kyle--getDebugInfo(start blob)-----');
            const ctx = await res.blob();
            console.log('------kyle--getDebugInfo(finish blob)-----');
            const re = /filename="(\S+)"/;
            console.log(res.headers.get('Content-Disposition').match(re));
            const filename = res.headers.get('Content-Disposition').match(re)[1];
            const result = {
